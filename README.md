...
SeaPass is organized into three folders at the repository root:

```
seapass/
├── web/          # Laravel 12 web application (REST API + web admin dashboard)
├── mobile/       # Flutter passenger app (Android)
└── scripts/      # Windows helper scripts (firewall, server, ADB forwarding)
```
...
SeaPass consists of two decoupled sub-systems, each kept in its own folder:

| Component | Path | Technology | Description |
|---|---|---|---|
| **Web App & API** | `web/` | Laravel 12, PHP 8.2+, MySQL 8+, Vite | RESTful JSON API handling authentication, bookings, schedules, QR validation, and admin web views. |
| **Mobile Client** | `mobile/` | Flutter 3.x, Dart 3.x, Android SDK | Android passenger app supporting QR ticketing, live schedule browsing, and manifest verification. |
| **Automation Scripts** | `scripts/` | Windows Batch & PowerShell | Auto-detects network IP, opens firewall ports, sets up ADB reverse port forwarding, and starts servers. |
...
This repository includes pre-built Windows batch scripts in the `scripts/` folder:

1. **Open Port 8000 in Windows Defender Firewall:**
   Right-click `scripts/setup_firewall.bat` and click **Run as administrator**.
...
When running wirelessly over Wi-Fi, update `lanWifiHost` in `mobile/lib/config/api_config.dart`:
```dart
...
3. In PowerShell, forward port 8000 from the phone to your PC:
   ...
   ```
   *(Or simply run `scripts/connect_usb.bat` from the project root).*
...
- **Windows Firewall Blocking:** Run `scripts/setup_firewall.bat` as Administrator, or manually open port 8000:
  ```powershell
